...
// 		$this->redirect('User/login');
	...
	public function isLogin($name='isLogin',$code=false){
		$user=SessionManage::getUser();
// 		$user=$this->_clientM->getCurUser();
		ee('isLogin usersession:'.getoutstr($user));
		if(!$code || !is_numeric($code) || 1==$code){
			$code=-1;///-2;//手机已经绑定过微信
		}
		if(!$user){
			$guest=array('userid'=>$code);
		}else{
...
// 					default: $error = '未知错误！'; break; // 0-接口参数错误（调试阶段使用）
	...
	public function loginByWx(){
		...
		ee('login third ret:'.getoutstr($loginInfo),'temp');
		$url='';
...
//存在登录成功 返回主页
			$user=SessionManage::getUser();
// 		$user=$this->_clientM->getCurUser();
			ee('isLogin usersession:'.getoutstr($user),'temp');
			$url='http://oneonebao.com/#home'; //oneonebao.com/#home';
		}elseif (2===$loginInfo ||  1===$loginInfo){  //
//授权成功，用户信息不存在 ，绑定账户  //或用户存在，手机不在
