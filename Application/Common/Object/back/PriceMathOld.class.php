...
// 		车身折旧价=车身价*（1-折旧率*折旧自然月数）
	...
	public function dsfzr(Insure $insure){
		...
		return $this->_catePrice->dsfzr=$price*$this->_dicountFactor;
	}
	//车损
...
// 		$level_map_index=Level::$clss[$this->_carTypeGroup][$this->get_convert_clss_index()];   ////old
		...
		return $this->_catePrice->clss=$price*$this->_dicountFactor;
	}
	//取到车损原始价
...
// 		基本保费+车身折旧价X费率
		...
		return $this->_catePrice->qcdq=$price*$this->_dicountFactor;
	}
	//司机
	public function sjzwzr(Insure $insure){
		...
		return $this->_catePrice->sjzwzr=$price*$this->_dicountFactor;
	}
	//乘客
	public function ckzwzr(Insure $insure){
		$price=0;
		$levelPrice=Level::$ckzwzr[$insure->_level] * 10000;//单位是万
		$price=$levelPrice*$this->_oneRate->ckzwzr[0]; 
		if($insure->_noPay){
			$price+=$price*0.15;
		}
		return $this->_catePrice->ckzwzr=$price*$this->_dicountFactor;
	}
	//玻璃      选择档次  1，国产   2，进口
	public function bl(Insure $insure){
		...
		return $this->_catePrice->bl=$price*$this->_dicountFactor;
	}
	//涉水
...
// 		必须购买车损险；车损险保费X5%
		...
		return $this->_catePrice->ss=$price * $this->_dicountFactor;
	}
	//划痕
	public function hh(Insure $insure){
		...
		return $this->_catePrice->hh=$price * $this->_dicountFactor;
	}
	//自燃
...
// 		$row=$this->getZrRow();  ///old
		...
		$price=$this->getCarDepreciationPrice() * $row[$this->getZrLevelIndex()];
		
		return $this->_catePrice->zr=$price * $this->_dicountFactor;
	}
	//指定
